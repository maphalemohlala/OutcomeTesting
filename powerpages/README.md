...
> with `restoretablepermissions` instead, then proves by query that all thirteen
> landed.
>
...
**Release gate** — `powerpages/Check-PortalSecurity.ps1` runs alongside `Check-ComponentIds.ps1` before every upload and asserts: no table permission grants an anonymous role anything; no `PROVISIONAL` permission survives; every business page is covered by a Restrict Read rule on itself or an ancestor; a child page's rule is a subset of its nearest ancestor's; at most one Restrict Read rule per page; no page rule binds Anonymous Users; self-registration and external login are both off; every web role a permission or a page rule references still exists in `webrole.yml`; no Global-scope table permission carries write, create or delete; and a Grant Change page rule may bind only the Administrators role. Both scripts read the repository, not the deployed site — a green gate does not by itself prove a row was deleted from DEV; see the deployment runbook's step on verifying deletions. `Deploy-Portal.ps1 -VerifyOnly` is the one that reads the **deployed** site: it queries `powerpagecomponent` type 18 and names every table permission that source declares and the environment does not have. That check is what caught DEV holding 2 of 13 on 2026-09-06, three weeks after the gates started passing.
...
- Global **read** is granted to Authenticated Users on `al_OutcomeCase`, `al_ReviewInstance`, `al_Response` and `al_RemediationAction` (OD-022, AD-056, AD-069) and on the reference tables `al_Section`, `al_QuestionVersion`, `al_Question` and `al_FailReason` (AD-047). `al_ChecklistVersion` is not among them — its PROVISIONAL permission was deleted in this branch, since no `ot-*` web template queries it (see the security closure design, section 3). No portal role gets Global **write**: every write path is reachable only through a Contact-scoped permission (`Review Instance - assigned to me`, `Response - on a review assigned to me`, `Remediation Action - assigned to me`) bound to the purpose-built `AL Portal` roles.
