...
**Assertion 12 — the template parses.** Two faults that upload cleanly and break the page at render time. A real Liquid tag inside a `{% comment %}` block is the first: DotLiquid tokenises tags inside a comment, so an illustrative `{% if %}` written in prose has no `{% endif %}`, swallows the `{% endcomment %}`, and the page dies with **unknown tag 'endcomment'** — naming a tag that is not the problem. That is why the templates here write `(% if %)` with parentheses in prose, a convention that was remembered rather than enforced until it was broken on `OT Remediation` on 2026-09-11. The second is an unbalanced block tag — removing an `{% unless %}` and leaving its `{% endunless %}` is the shape that survives review. Both are checked by walking the tags in order. Counting opens against closes passes on a file where they interleave wrongly, and stripping comments with a regex first — the obvious way to do it by hand, and the way that missed this — pairs the first `{% comment %}` with the first `{% endcomment %}` and hides the fault it is looking for.
