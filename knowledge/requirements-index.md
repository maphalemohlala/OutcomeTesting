...
| MR-nn | 8 September 2026 meeting requirement | `docs/reference/2026-09-08-solution-requirements.md` |
...
| PP-12 | Remediation workspace: adviser response and T&C Manager attestation, both in the portal | BR-006, BR-008, FR-020 to FR-023, AD-045 | **Both halves built 2026-09-02.** Adviser response, Intelligent Office reference and completion, inline on `/remediation`; then T&C approval or rejection with notes, where a rejection reopens the action and an approval advances the case. The two halves use different mechanics on purpose (AD-074): completion is an update behind a trigger column, attestation is a *create* on `al_signoff` carrying its own permission, so an adviser cannot approve their own remediation through the shared field allowlist. `SignoffGuardPlugin` validates and stamps, `SignoffProgressPlugin` applies the consequences. **Two later changes this row had not carried.** (i) The AD-074 browser-side create was superseded on 2026-09-09 by AD-099: the page PATCHes a trigger column on the signatory own contact and `SignoffRequestPlugin` checks the web roles server-side and creates the `al_signoff`. It was found when a verified T&C Supervisor was answered 403 and the environment held **zero** `al_signoff` rows — no sign-off had ever succeeded. The AD-074 permission stays for reads. (ii) Since AD-107 a review raises one action per failed item, so attestation is per item and `RemediationApproved` requires every action on the review to be approved, not any one |
...
| PP-15 | Nine notification events emitted via an outbox; email only, delivered server-side | BR-009, FR-021, AD-035 | **Built, switched on and PROVED END TO END 2026-09-05 for the five enumerated events; four events remain unnamed.** The outbox, all five emitters and now the drain are built: an asynchronous post-operation Create step on `al_notification` sends through Dataverse server-side email from the account the step runs as — no Power Automate, no connector licence (AD-081). `Review submitted` routes to the case's para-planner, closing OD-030(ii) and carrying BR-009 (AD-082). A send that fails ends at `Failed` with `al_failurereason` and stays retryable through `al_DrainNotifications`. **Deployed to DEV 2026-09-04 (`9390177`), switched on 2026-09-05, and proved the same evening.** The 09-04 position — drain step unregistered, mailbox unapproved, BR-009 riding on the switch — is superseded on every count. The mailbox reads `isemailaddressapprovedbyo365admin = Yes` and `outgoingemailstatus = Success`; the drain step is registered, asynchronous and enabled, running as `svc.automate.aq@ascotlloyd.co.uk`; and an allocation raised in DEV travelled emitter → outbox row → asynchronous drain → server-side email → **delivered**, twice. Evidence: two `al_notification` rows at `Sent`, each with an Outgoing/`Sent` email activity and an Incoming/`Received` copy of the same message tracked back into the service mailbox. Reproduce with `provepp15`, read the standing evidence with `pp15evidence`, both in `plugins/OutcomeTesting.Registration`. Until this run the whole path had been verified only in pieces and `al_notification` held zero rows, so nothing had ever travelled it — "switched on" and "working" were genuinely different claims. **Operational prerequisite, unchanged:** server-side email needs an approved, tested mailbox per environment, and **registering the drain step is the per-environment gate** — until an environment's mailbox is confirmed the step is not registered, rows rest at `Pending`, and nothing claims an email was sent that was not. **Still short of the requirement as written:** PP-15 says nine events and five are built, because the other four are enumerated in no requirement, knowledge file or design document (OD-030 gap (a), answered 2026-09-03 by direction to build the five). Adding the rest stays additive. |
| PP-16 | User-friendly errors; technical detail and correlation ID logged separately, never surfaced | NFR-OBS-01 | Confirmed |
| PP-17 | Reporting drill-down from outcome cards to filtered lists; full MI and export remain back-office | BR-010, FR-032 | **Built 2026-09-04 (portal).** Five outcome cards on portal Home — the four BR-005 grades plus Not yet graded — each linking to `/cases?outcome=<slug>`, with a matching Outcome filter and Outcome column added to `ot-case-list`. The grade in force is final where recorded, otherwise initial (BR-007), expressed the same way in the card aggregates and the list filter; the `al_outcome` join is outer so ungraded cases are not dropped from the worklist. Grades render through the new `OT Outcome Label` template, never as option-set numbers. Full MI and the Trail Light export stay in the Code App, which had its own drill-down from `b6d5583` — that was the Code App's, not the portal's, and did not meet PP-17. See AD-083. |

## 8 September 2026 meeting requirements

Source: `docs/reference/2026-09-08-solution-requirements.md`, supplied by the project owner
2026-09-09, consolidating the 8 September meeting and the workflow demonstration. Assessed
against the built solution 2026-09-10: `docs/2026-09-10-requirements-gap-assessment.md`.

**This pack is mostly a re-statement of requirements already registered above.** It is given its
own ID band so the project owner's priority ordering is quotable, not because it introduces a
second requirements authority. Where an MR maps to an existing ID, that ID stays the one the
build traces to; where the pack asks for something genuinely new, it raises an OD rather than
being implemented from the prose.

| ID | Pack item | Traces to | State 2026-09-10 |
|---|---|---|---|
| MR-01 | Mirror V8 checklist structure and ordering | BR-001 range, FR-010 to FR-013, PP-07, PP-08, PP-09, AD-098, AD-101 to AD-106 | **Built**, and pinned by an executable test against the supplied reference file (AD-104). `checklist-v8.md` and `OT-Review-Detail` still carry uncommitted edits. §4.1's *subsection* half is presentation only — **OD-039** |
| MR-02 | Failed items populate remediation automatically | BR-006, FR-020, AD-097, AD-106, AD-107 | **Built.** Raised at all only from 2026-09-09 — until that day nothing in the solution had ever created an `al_remediationaction`. **One action per failed item since 2026-09-10** (AD-107), which is what makes §6.1's per-item fields expressible; OD-040 is resolved |
| MR-03 | Remediation form redesigned around required actions | FR-020 to FR-022, PP-12, PP-14, AD-095, AD-107 | **Built and committed.** The three choice columns on `al_RemediationAction`, each fail item drawn as its own answerable row, and a portal list of cases in remediation that opens one to its form |
| MR-04 | Supervisor sign-off and final outcome regrading | BR-008, FR-023, FR-024, PP-12, AD-099 | **Partial.** Sign-off works, and only since 2026-09-09 — the environment held zero `al_signoff` rows before it. Regrade exists but only in the Code App — **OD-041** |
| MR-05 | Initial and final outcomes tracked separately | BR-007, PP-10, AD-031 | **Built and deployed.** `al_initialoutcome` and `al_finaloutcome`; a final never overwrites an initial |
| MR-06 | Portal, account and role permissions resolved | BR-012, PP-01, PP-02, NFR-SEC-01, AD-085 to AD-088 | **Partial.** Rebuilt onto web roles; several 403s found and fixed as late as 2026-09-09. One contact holds reviewer *and* manager roles, so role separation is untested — see `docs/2026-09-04-outstanding-work.md` §1.1 |
| MR-07 | Route to the first authorised page | NFR-SEC-01, PP-02 | **Not built.** `/` is ungated and every role is seeded with dashboard View — **OD-042** |
| MR-08 | Remediation form alignment and styling | NFR-ACC-01, PP-16, AD-107 | **Built and committed** — each fail item drawn as its own row (`93571bf`) and a portal remediation list that opens a case to its form (`06380ec`) |
| MR-09 | Verify reviewer assignment and submission rules | FR-004 to FR-006, FR-017, PP-11, AD-076 | **Built.** The 2026-09-09 "assigned but not on My Work" report was a user editing the checker *name* rather than allocating |
| MR-10 | Corrected solution prepared in the test environment | OD-011, and item 6 of the outstanding-work register | **Not started**, and sequenced behind DEV sign-off by project owner direction 2026-09-06 |
| MR-11 | Email notifications on approved templates | BR-009, PP-15, AD-035, AD-081, AD-082 | Mechanism **built, switched on and proved end to end** in DEV. Templates and wording are business input — **OD-046** |
| MR-12 | Direct case or action links in notifications | PP-15, §9.2 | **Built.** `NotificationOutbox.CaseLink` resolves the site domain per environment |
| MR-13 | Simplified dashboard and drill-down | PP-03, PP-17, AD-083 | Drill-down **built**; simplification **in flight**. §3.4's initial-vs-final split — **OD-043** |
| MR-14 | Remediation and checklist navigation | PP-16, §14.1 | **Partial.** Portal block headings landed 2026-09-09 (AD-105) |
| MR-15 | Structured defect recording during UAT | none — new | **Not built**, no mechanism named — **OD-045** |
| MR-16 | Direct Power BI or database reporting | OD-012, AD-034 | Closed as out of MVP scope by direction; the pack re-raises it as **backlog**, which is consistent. §8.5 asks only that it be documented and prioritised |
| MR-17 | Extended management reporting | BR-010, FR-032, §8.2 | Backlog. The operational areas §8.2 lists are partly served by `/reports` and the export today |
| MR-18 | Trail Light automation and reconciliation | BR-011, OD-005, AD-034, AD-039, AD-042, AD-075 | Export **built and proved** (3 rows, both graded columns). Automated transfer stays out of MVP scope. §13.1's validation *by Trail Light* has not happened — the contract is verified against the supplied map, not against a Trail Light acceptance run |
...
| FR-024 | **Recheck and regrade built 2026-09-03** in the Code App at `/cases/:caseId/recheck`. The T&C Manager sets the final outcome with a mandatory reason through `al_RegradeCase`; the initial outcome is preserved so both survive (BR-007, OD-007, AD-031). The screen is gated on `page.cases` for view and `command.regrade` for the action, and passes the outcome's row version as `ExpectedRowVersion` so a regrade against a stale read is refused rather than overwriting another correction. The OD-018 clock reset on a rejected sign-off was built separately the same day (AD-079) **Also on the portal from 2026-09-10 (AD-111, resolving OD-041):** the supervisor records the same final outcome beside the sign-off on `OT Remediation`, through `contact.al_regraderequest` and `RegradeRequestPlugin`, which calls the same `RegradeCasePlugin.Regrade` the Custom API calls. A Custom API cannot be invoked from a Power Pages page, so until then a supervisor holding only the portal could approve every action and then not close the case. |
