...
Content is administered in the version already in force rather than by reissuing the checklist, and nothing is ever deleted: a question is taken out of service by dating out its current Question Version, and a section by dating out the section (AD-122, AD-123). Sections carry `al_effectivefrom`, `al_effectiveto` and `al_isoptional`, and are owned by Tax, AQS or **Both** — a Both section is rendered to and owed by each discipline, which answers its own copy. Effective dates are what protect history: every read path is date-scoped and a submitted review reads as of its submission day (AD-091), so adding or retiring content cannot change what a submitted review said. Eight question codes are load-bearing in the plug-in assembly and are refused retirement by name (AD-122).
